		...
		private static function _StripLinkDefinitions(text:String):String {
			...
			*/
			text = text.replace(/^[ ]{0,3}\[(.+)\]:[ \t]*\n?[ \t]*<?(\S+?)>?[ \t]*\n?[ \t]*(?:(\n*)["(](.+?)[")][ \t]*)?(?:\n+|\Z)/gm,
				function (wholeMatch:String, m1:String, m2:String, m3:String, m4:String, ...rest):String {
					m1 = m1.toLowerCase();
...
//			trace('text', text);
		...
		private static function _RunBlockGamut(text:String):String {
			...
			//
			text = _DoHeaders(text); //FIXME uncomment this when finished
...
//			var regex:RegExp = /(<[a-z\/!$]("[^"]*"|'[^']*'|[^'">])*>|<!(--.*?--\s*)+>)/gi;
			...
			text = text.replace(/(<[a-z\/!$]("[^"]*"|'[^']*'|[^'">])*>|<!(--.*?--\s*)+>)/gi, function(wholeMatch:String, ...rest):String {
		...
		private static function _DoHeaders(text:String):String {
			...
			//
			text = text.replace(/^(.+)[ \t]*\n=+[ \t]*\n+/gm,
				function(wholeMatch:String, m1:String, ...rest):String{return hashBlock("<h1>" + _RunSpanGamut(m1) + "</h1>");});
			
			text = text.replace(/^(.+)[ \t]*\n-+[ \t]*\n+/gm,
				function(matchFound:String, m1:String, ...rest):String{return hashBlock("<h2>" + _RunSpanGamut(m1) + "</h2>");});
			...
			*/
			...
			text = text.replace(/^(\#{1,6})[ \t]*(.+?)[ \t]*\#*\n+/gm,
				function(wholeMatch:String, m1:String, m2:String, ...rest):String {
...
//			text += "~0";
			...
			*/
			...
			if (g_list_level > 0) {
//				trace('hit>>>', text);
				text = text.replace(whole_list, function(wholeMatch:String, m1:String, m2:String, ...rest):String {
					var list:String = m1;
					var list_type:String = (m2.search(/[*+-]/g)>-1) ? "ul" : "ol";
//					trace('going to process', list, list_type);
				...
				whole_list = /(\n\n|^\n?)(([ ]{0,3}([*+-]|\d+[.])[ \t]+)[^\r]+?(~0|\n{2,}(?=\S)(?![ \t]*(?:[*+-]|\d+[.])[ \t]+)))/g;
				text = text.replace(whole_list,function(wholeMatch:String, m1:String, m2:String, m3:String, ...rest):String {
					var runup:String = m1;
					var list:String = m2;
		...
		private static function _DoBlockQuotes(text:String):String {
			...
			*/
			...
			text = text.replace(/((^[ \t]*>[ \t]?.+\n(.+\n)*\n*)+)/gm,
				function(wholeMatch:String, m1:String, ...rest):String{
					var bq:String = m1;
...
//					var blockText:String = g_html_blocks[RegExp.$1]; //FIXME this is a shitty bit
//					blockText = blockText.replace(/\$/g,"$$$$"); // Escape any dollar signs
//					grafsOut[i] = grafsOut[i].replace(/~K\d+K/,blockText);
//				}
				
//				var aaa:Array = String(grafsOut[i]).match(/~K(\d+)K/g);
//				trace(aaa);
//				for each(var sss:String in aaa) {
//					var id:int = sss.search(
//					var blockText:String = g_html_blocks[int(sss)]; //FIXME this is a shitty bit
//					blockText = blockText.replace(/\$/g,"$$$$"); // Escape any dollar signs
//					grafsOut[i] = grafsOut[i].replace(/~K\d+K/, blockText);
//				}
		...
		private static function _EncodeEmailAddress(text:String):String {
			...
			addr = addr.replace(/./g, function(ch:String, ...rest):String {
				if (ch == "@") {
		...
		private static function hashElement(wholeMatch:String, m1:String, ...rest):String {
		...
		private static function writeAnchorTag(wholeMatch:String, m1:String, m2:String, m3:String, m4:String, m5:String, m6:String, m7:String = '', ...rest):String {
		...
		private static function writeImageTag(wholeMatch:String, m1:String, m2:String, m3:String, m4:String, m5:String, m6:String, m7:String = '', ...rest):String {
			var whole_match:String = m1;
		...
		private static function escapeCharacters_callback(wholeMatch:String, m1:String):String {
